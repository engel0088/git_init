...
tinyMCE.importPluginLanguagePack('imageupload', 'en,tr,he,nb,ru,ru_KOI8-R,ru_UTF-8,nn,fi,cy,es,is,pl'); // <- Add a comma separated list of all supported languages

/****
 * Steps for creating a plugin from this imageupload:
 *
 * 1. Change all "imageupload" to the name of your plugin.
...
var TinyMCE_ImageuploadPlugin = {
	...
	getInfo : function() {
		return {
			...
			infourl : 'http://www.yoursite.com/docs/imageupload.html',
			version : "1.0"
		};
	},
	...
	initInstance : function(inst) {
		...
		//alert("Initialization parameter:" + tinyMCE.getParam("imageupload_someparam", false));

		// Register custom keyboard shortcut
		inst.addShortcut('ctrl', 't', 'lang_imageupload_desc', 'doUpload');
	},*/
	...
	getControlHTML : function(cn) {
		switch (cn) {
			case "imageupload":
				return tinyMCE.getButtonHTML(cn, 'lang_imageupload_desc', '{$pluginurl}/images/imageupload.gif', 'doUpload', true);
		}
		...
		switch (command) {
			...
			case "doUpload":
				...
				if (user_interface) {
					...
					imageupload['file'] = '../../plugins/imageupload/upload_form.html'; // Relative to theme
					imageupload['width'] = 300;
					imageupload['height'] = 200;

					tinyMCE.openWindow(imageupload, {editor_id : editor_id, some_custom_arg : "somecustomdata"});

					// Let TinyMCE know that something was modified
					tinyMCE.triggerNodeChange(false);
				} else {
					// Do a command this gets called from the imageupload popup
					alert("execCommand: doUpload gets called from popup.");
				}
	...
	handleNodeChange : function(editor_id, node, undo_index, undo_levels, visual_aid, any_selection) {
		// Select imageupload button if parent node is a strong or b
		if (node.parentNode.nodeName == "STRONG" || node.parentNode.nodeName == "B") {
			tinyMCE.switchClass(editor_id + '_imageupload', 'mceButtonSelected');
			return true;
		}

		// Deselect imageupload button
		tinyMCE.switchClass(editor_id + '_imageupload', 'mceButtonNormal');
	},
	...
	handleEvent : function(e) {
		...
		top.status = "imageupload plugin event: " + e.type;
...
tinyMCE.addPlugin("imageupload", TinyMCE_ImageuploadPlugin);
